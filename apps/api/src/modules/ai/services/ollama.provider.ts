...
export class OllamaProvider extends AiProvider {
  ...
  async analyzeImage(
    ...
    try {
      // Ollama: images BEFORE text in content array (Gemma4 multimodal best practice)
  ...
  async processAudio(audioBuffer: Buffer, _mime: string): Promise<AudioResult> {
    try {
      // Ollama API does not support audio — delegate to Whisper container
      ...
      return { text: response.data.text ?? '', language: response.data.language ?? 'auto' };
    } catch (err) {
