// apps/api/src/modules/ai/providers/gemini.strategy.ts
//
// CHANGE vs original:
//   processAudio() now validates the transcript before returning.
//   When Gemini receives silent audio it produces SRT timestamps
//   ("00:00 00:01 00:02 ...").  The strategy now detects this and
//   returns { text: '', language: 'auto' } so the caller (IntentExtractorService)
//   can short-circuit without a wasted second LLM call.
...
//
// Gemini produces SRT-style timestamp sequences when given silent or
// near-silent audio.  Example:
//   "00:00 00:01 00:02 00:03 00:00 00:01 00:02 00:03 00:01 00"
//
// Detecting this at the strategy layer (before returning to the service) means
// the caller gets { text: '', language: 'auto' } and can apply a fast
// short-circuit instead of performing a second, equally wasted LLM call.
...
export class GeminiStrategy implements IAiProvider {
  ...
  async processAudio(
    ...
    try {
      ...
      // Return an explicit empty transcript instead of timestamp garbage so
      // IntentExtractorService can fast-path to FALLBACK without a second
      // wasted LLM call.
